...
struct kmem_cache* cache_table1(size_t size)
{
        struct h_node *cur;
...
{
        struct kmem_cache * cache = cache_table1(size);
        if (cache == NULL) {        
                char name[32];
                struct h_node* hash_node = kmem_cache_alloc(hash_cache, GFP_KERNEL);
...
{
  char name[32];
  sprintf(name, "wine-cache-for-hash-%d",sizeof(struct h_node) );
